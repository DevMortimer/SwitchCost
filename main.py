import os, time, signal, logging, daemon, sys, datetime, ndjson
import tracker
...
def run_main_loop():
    ...
    while running:
        ...
        if tracker.current_prog != current_program:
            filepath = f'/tmp/SwitchCost/events-{str(datetime.datetime.today()).split()[0]}.ndjson'
            ...
            with open(filepath, 'a') as f:
                ...
                writer.writerow({
                    ...
                    'to': current_program,
                    'duration': str(now - tracker.get_last_timestamp(filepath)) if last else "0",
                })
...
def main() -> None:
    ...
    foreground = os.getenv("SWITCHCOST_FOREGROUND") == "1"
    ...
    context = daemon.DaemonContext(
        working_directory="/tmp/SwitchCost",
        umask=0o002,
        pidfile=PIDLockFile("/tmp/SwitchCost/SwitchCost.pid"),
    )
    context.signal_map = {
